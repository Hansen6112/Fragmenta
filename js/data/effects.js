...
const EFFECT_DEFS = {
  ...
  stones_patience: { name: "Stone's Patience", category: "regalia", description: "Whenever you do not move first in a combat round, your next attack deals 30% additional damage. Currently inert — this engine's combat always has the player act first each round (choosing an action, which resolves, before the enemy's retaliation), by construction; there is no scenario in which the player 'does not move first,' so this condition can never occur, not merely 'unbuilt.'" },
  ...
  unhurried_step: { name: "Unhurried Step", category: "regalia", description: "Enemy Haste, Speed, or Turn-order manipulation has no effect on you. Currently inert — no enemy Haste/Speed/turn-order mechanic exists in this engine (a strictly player-action-then-enemy-retaliation single-action-per-round model); deferred to the promised future enemy rework, the same standing reason as every other CC-immunity effect so far." },
